...
class CustomDPOTrainer(DPOTrainer):
    ...
    def compute_preference_loss(
        ...
        q_chosen: Optional["torch.Tensor"] = None,
        q_rejected: Optional["torch.Tensor"] = None,
        ...
        if not self.finetuning_args.use_ref_model:
            ...
            losses, chosen_rewards, rejected_rewards = self.dpo_loss(
                policy_chosen_logps, policy_rejected_logps, reference_chosen_logps, reference_rejected_logps, q_chosen, q_rejected
            )
            ### [Edited!!!] ###
    ...
    def get_batch_loss_metrics(
        ...
        ### [Edited!!!] ###
        batch_wo_q = {k: v for k, v in batch.items() if k != "q_value"}
        metrics = {}
        (
            policy_chosen_logps,
        ...
        ) = self.concatenated_forward(model, batch_wo_q)

        reference_chosen_logps, reference_rejected_logps = self.compute_reference_log_probs(model, batch_wo_q)
        if batch.get("q_value") is not None:
            batch_size = batch["input_ids"].size(0) // 2
            q_chosen, q_rejected = batch['q_value'].split(batch_size, dim=0)
    ...
    def dpo_loss(
        ...
        q_chosen: torch.FloatTensor = None,
        q_rejected: torch.FloatTensor = None,
        ...
        ### [edited!!!] ###
        elif "sigmoid_a" in self.loss_type:
            alpha = float(self.loss_type.split("_")[-1])
            ad = alpha * (q_rejected - q_chosen)
            losses = (
                -F.logsigmoid(self.beta * logits + ad) * (1 - self.label_smoothing)
                - F.logsigmoid(-self.beta * logits - ad) * self.label_smoothing
            )
        ### [edited!!!] ###
