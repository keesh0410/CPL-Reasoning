...
class PairwiseDataCollatorWithPadding(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: Sequence[Dict[str, Any]]) -> Dict[str, "torch.Tensor"]:
        ...
        if features[0].get("q_chosen") is not None and features[0].get("q_rejected") is not None:
            batch["q_value"] = []
            for key in ("chosen", "rejected"):
                for feature in features:
                    batch["q_value"].append(feature["q_{}".format(key)])
            batch["q_value"] = torch.tensor(batch["q_value"])
        ### [Edited!!!] ###
