...
def convert_alpaca(
    ...
    for i in range(len(examples[dataset_attr.prompt])):
        ...
        if dataset_attr.kto_tag and isinstance(examples[dataset_attr.kto_tag][i], bool):  # kto example
            ...
            ### [Edited!!!] ###
            if "q_chosen" in examples.keys() and "q_rejected" in examples.keys():
                if "q_chosen" in outputs.keys() and "q_rejected" in outputs.keys():
                    outputs["q_chosen"].append(examples["q_chosen"][i])
                    outputs["q_rejected"].append(examples["q_rejected"][i])
                else:
                    outputs["q_chosen"] = [examples["q_chosen"][i]]
                    outputs["q_rejected"] = [examples["q_rejected"][i]]
            ### [Edited!!!] ###
...
def align_dataset(
    ...
    if "q_chosen" in column_names and "q_rejected" in column_names:
        column_dict["q_chosen"] = {"dtype": "float32", "_type": "Value"}
        column_dict["q_rejected"] = {"dtype": "float32", "_type": "Value"}
